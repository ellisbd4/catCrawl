...
class DogSprite(wx.Frame):
    def __init__(self, image_path):
        ...
        self.Bind(wx.EVT_WINDOW_CREATE, self.SetShape)  # Set the shape once the window is created
    ...
    def OnPaint(self, event):
        dc = wx.PaintDC(self)
        dc.DrawBitmap(self.dog_image, 0, 0, True)

    def SetShape(self, event=None):
        """Set the shape of the window to match the image's alpha channel."""
        r = wx.Region(self.dog_image, wx.Colour(0, 0, 0, 0))  # Assuming black is the transparency color
        self.SetShape(r)
    ...
    def on_mouse_leave(self, event):
        ...
        self.should_move = True
        self.dog_image = self.current_sprites[self.sprite_index]  # reset to the last walking sprite
        self.Refresh()
        self.crawl_along_bottom()
